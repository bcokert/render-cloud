...
func Specular(reflectedVector, viewerVector mgl64.Vec3, shininess float64) (float64, error) {
	if !utils.IsDirectionVector(reflectedVector) {
		return 0, errors.New("illumination.Specular requires reflectedVector to be a direction vector, received the vector [0, 0, 0]")
	}
	if !utils.IsDirectionVector(viewerVector) {
		return 0, errors.New("illumination.Specular requires viewerVector to be a direction vector, received the vector [0, 0, 0]")
	}
...
func Diffuse(specularComponent float64, lightVector, normalVector mgl64.Vec3) (float64, error) {
	if !utils.IsDirectionVector(lightVector) {
		return 0, errors.New("illumination.Diffuse requires lightVector to be a direction vector, received the vector [0, 0, 0]")
	}
	if !utils.IsDirectionVector(normalVector) {
		return 0, errors.New("illumination.Diffuse requires normalVector to be a direction vector, received the vector [0, 0, 0]")
	}
...
func ViewerVector(ray mgl64.Vec3) (mgl64.Vec3, error) {
	if !utils.IsDirectionVector(ray) {
		return mgl64.Vec3{}, errors.New("illumination.ViewerVector requires ray to be a direction vector, received the vector [0, 0, 0]")
	}
...
func LightVector(light model.Light) (mgl64.Vec3, error) {
	if !utils.IsDirectionVector(light.Direction) {
		return mgl64.Vec3{}, errors.New("illumination.LightVector requires light.Direction to be a direction vector, received the vector [0, 0, 0]")
	}
...
func ReflectedVector(normalVector, lightVector mgl64.Vec3) (mgl64.Vec3, error) {
	if !utils.IsDirectionVector(lightVector) {
		return mgl64.Vec3{}, errors.New("illumination.LightVector requires lightVector to be a direction vector, received the vector [0, 0, 0]")
	}
	if !utils.IsDirectionVector(normalVector) {
		return mgl64.Vec3{}, errors.New("illumination.LightVector requires normalVector to be a direction vector, received the vector [0, 0, 0]")
	}
...
func (this PhongIlluminator)  IlluminateLocal(ray, normalVector mgl64.Vec3, material materials.Material, world model.World) (colorful.Color, error) {
	...
	for _, light := range world.Lights {
		...
		if viewerVector, err = ViewerVector(ray); err != nil {
			return colorful.Color{}, errors.New("illumination.ViewerVector Failed: " + err.Error())
		}

		if lightVector, err = LightVector(light); err != nil {
			return colorful.Color{}, errors.New("illumination.LightVector Failed: " + err.Error())
		}

		if reflectedVector, err = ReflectedVector(normalVector, lightVector); err != nil {
			return colorful.Color{}, errors.New("illumination.ReflectedVector Failed: " + err.Error())
		}

		if specularComponent, err = Specular(reflectedVector, viewerVector, material.Shininess); err != nil {
			return colorful.Color{}, errors.New("illumination.Specular Failed: " + err.Error())
		}

		if diffuseComponent, err = Diffuse(specularComponent, lightVector, normalVector); err != nil {
			return colorful.Color{}, errors.New("illumination.Diffuse Failed: " + err.Error())
		}
